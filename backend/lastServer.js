    ...
    for (let section of course.sections) {
      const lessonsResult = await pool.query(
        `SELECT id, title, description, lesson_type, order_index,
                is_free, video_duration, pdf_path, created_at
         FROM lessons
...
let startIndex = maxOrderResult.rows[0].max_order + 1;
  ...
  try {
    ...
    for (let i = 0; i < questions.length; i++) {
      const { question_text, options, correct_answer, question_type } = questions[i];

      const result = await pool.query(
        `INSERT INTO quiz_questions (quiz_id, question_text, options, correct_answer, order_index, question_type)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING id, question_text, options, correct_answer, order_index, question_type`,
        [quizId, question_text, JSON.stringify(options), correct_answer, startIndex + i, question_type || 'single_choice']
      );
...
app.get("/api/quizzes/:quizId", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const questions = await pool.query(
      `SELECT id, question_text, options, order_index, question_type
       FROM quiz_questions
       WHERE quiz_id = $1 AND NOT is_deleted
       ORDER BY order_index ASC`,
      [quizId]
    );
...
app.post("/api/quizzes/:quizId/submit", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const questions = await pool.query(
      `SELECT id, correct_answer, question_type FROM quiz_questions WHERE quiz_id = $1 AND NOT is_deleted`,
      [quizId]
    );
    ...
    questions.rows.forEach(q => {
      ...
      if (qType === 'text') {
        textAnswers.push({ question_id: q.id, answer: answers[q.id] || '' });
      } else {
        autoGradable++;
    ...
    const score = autoGradable > 0 ? Math.round((correct / autoGradable) * 100) : 0;
    const needsGrading = textAnswers.length > 0;
...
const enrollmentId = enrollmentRes.rows[0].id;
    ...
    // 6. Αν score >= passing_score ΚΑΙ δεν υπάρχουν text ερωτήσεις, έκδοση certificate
    let certificate = null;
    if (score >= passing_score && !needsGrading && enrollment.rows.length > 0) {
    ...
    const passed = score >= passing_score && !needsGrading;
    const message = needsGrading
      ? "Οι απαντήσεις σου υποβλήθηκαν! Κάποιες ερωτήσεις αξιολογούνται από τον καθηγητή."
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const { data } = await drive.files.create({
        requestBody: {
          name: `pdf_lesson_${lessonId}_${Date.now()}.pdf`,
          mimeType: 'application/pdf',
          parents: [process.env.PDF_FOLDER_ID]
        },
        media: {
          mimeType: 'application/pdf',
      ...
      const pdfUrl = `https://drive.google.com/file/d/${data.id}/preview`;

      await pool.query(
        `UPDATE lessons SET pdf_path = $1 WHERE id = $2 AND NOT is_deleted`,
        [pdfUrl, lessonId]
      );
...
app.get('/api/users/chat-list', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const query = `
      ...
      WHERE u.id != $1 AND NOT u.is_deleted AND u.role != 'pending_lecturer'
